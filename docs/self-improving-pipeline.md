...
- **Human interaction is tracked as a metric.** Every time the `BMAD Session (OpenCode)` question form fires, it counts as a halt. The count is journaled as `halts` on the `step_end` event and aggregated per-step and per-story in the trends. The reflection prompt treats halts as an autonomy signal: if a step keeps halting for the same decision class, the reflection records a `decision-policy-candidate-*` observation (for a human to add a rule) or proposes an `update_step` to tune the step's prompt toward the decision policy. Fewer halts over time means the pipeline is becoming more autonomous.
