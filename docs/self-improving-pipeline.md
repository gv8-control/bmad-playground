...
| Runner errors | `_bmad-output/pipeline/runner-errors.jsonl` | Machinery failures captured by the `BMAD Session (OpenCode)` wrapper: exit code, stderr tail, error category. |
...
- **Human interaction is tracked as a metric.** Every time the `BMAD Session (OpenCode)` question form fires, it counts as a halt. The count is journaled as `halts` on the `step_end` event (always present, even when 0) and the outcome sequence is journaled as `outcomeHistory`, aggregated per-step and per-story in the trends. INCOMPLETE outcomes (agent quit mid-stream) auto-continue with a "Continue" message until the agent produces COMPLETE or QUESTION; the count is journaled as `incompleteCount` on the `step_end` event. The reflection prompt treats halts as a signal to investigate, not an autonomy verdict: it first determines what caused the form to fire — a genuine uncovered decision (recorded as a `decision-policy-candidate-*` observation) or an infrastructure failure misclassified as a question (recorded as an `infra-*` observation). Fewer genuine halts over time means the pipeline is becoming more autonomous; phantom halts mean the machinery needs attention.
