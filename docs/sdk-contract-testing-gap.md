...
- Should the recording script live in `apps/agent-be/test/` or a separate `scripts/` directory? — Lives at `apps/agent-be/test/fixtures/record-session.ts`.
- Should the JSONL fixture be committed to the repo (large, but reproducible) or generated by a documented procedure and gitignored? — Committed at `apps/agent-be/test/fixtures/sdk-session-replay.jsonl`.
- Does this warrant a project-context.md pattern entry so future stories follow it by default? — No project-context.md pattern entry was added; the recorded-session approach is documented in this gap doc and the architecture.

## Resolution

The proposed mitigation is fully implemented and operational as an automated test in CI:

- The recorded-session replay test is implemented at `apps/agent-be/test/sdk-contract-replay.spec.ts`.
- The JSONL fixture is at `apps/agent-be/test/fixtures/sdk-session-replay.jsonl` (a real recorded session, not fabricated).
- The recording script is at `apps/agent-be/test/fixtures/record-session.ts`.
- The `Query`-shaped mock with `interrupt()` spy is at `apps/agent-be/test/helpers/mock-query.ts`.
- Type-checked `SDKMessage` fixture builders are in `apps/agent-be/src/streaming/agent.service.unit.spec.ts` (enforced by the `agent-be:typecheck` CI gate).
- The architecture's prescribed safeguard (`architecture.md:80`) is now operationalized as an automated test.
