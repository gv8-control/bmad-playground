...
### Hedged language doesn't survive the artifact handoff

**The temptation (2026-07-17):** An implementation artifact introduced an external binary's CLI invocation with hedged language: "The command likely looks like: `sandbox-agent --agent claude-code --prompt "<message>"` or similar." The hedge ("likely... or similar") was an honest confession of ignorance — the developer had not verified the binary's interface. The next story's artifact extracted the command string and left the hedging prose behind. The bare string became a design decision. It was codified as production code, "verified" by 44+ test assertions that hardcoded the same string, and shipped as `done` — all without anyone checking whether `--agent` or `--prompt` are real flags. They are not; the binary is an HTTP server, not a stdout-emitting CLI. The truth was documented in pre-epic research and re-discovered during implementation — both times recorded as a Decision Point and ignored in favor of the placeholder.

**Why it fails:** Uncertainty expressed in language stays in the originating document; the claim it hedged travels forward as a bare assertion. Each downstream artifact reads the command string, not the surrounding prose. The hedge is a signal — "this is unverified" — but signals don't survive extraction. By the time the string reaches production code, every trace of "likely" and "or similar" has been stripped. The code comment said "a reasonable invocation based on sandbox-agent docs" — no docs were cited, and the research docs in the codebase said the opposite. The phrase "based on docs" functioned as an unfalsifiable assertion, not an evidence claim. Meanwhile the tests — generated from the spec that contained the hallucination — hardcoded the same string the code hardcodes, creating a closed loop: code matches spec, tests match code, ACs match tests. All internally consistent; none checked against the real binary.

**Trap's shape:** LLMs naturally produce hedged language when uncertain ("likely," "probably," "or similar," "should be"). This is honest signaling. But artifact chains extract claims from their surrounding prose — a command string, a parameter name, a return type — and the hedge doesn't travel with the claim. The uncertainty is lost at the first handoff, and every subsequent artifact treats the claim as resolved. The richer the surrounding context (detailed JSDoc, architectural rationale, test coverage), the more the unverified claim feels grounded. That detail is borrowed confidence, not evidence — and the test suite, which derives its assertions from the same spec, will "verify" the hallucination rather than catch it.

**What we did instead:** Filed a sprint change proposal naming the hallucination and its full propagation path; scheduled the transport rewrite (Epic 8).

**Source:** Epic 6 — Story 6.2 artifact introduced `sandbox-agent --agent claude-code --prompt` as "likely... or similar"; Story 6.3 codified it as production code (`agent.service.ts:450`); 44+ tests hardcoded the string; Story 6.5 (verification gate) never ran; Epic 6 marked `done`. Truth known in pre-epic research (`technical-backend-service-architecture-...-research-2026-06-12.md:283`) and re-discovered in Story 6.2 Task 1 (Finding 1 + DP-2).
