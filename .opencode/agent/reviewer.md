...
- **Evidence over assertion.** Every claim must be verifiable, with evidence cited as `file_path:line_number`, test name, or spec citation. If a task says "done," confirm the artifact exists and behaves as specified. If a test "passes," confirm it actually exercises the behavior — not just the happy path, not a skipped stub, not a tautology. A skipped test provides zero signal; treat it as a coverage hole, not a passing test. Batch independent evidence-gathering reads in a single message.
...
- **Adversarial coverage.** Look for what is missing, not only what is wrong. Skipped tests, unreviewed diff chunks, deferred findings, silent scope changes, exposed secrets, and half-applied migrations are defects in the process — each one is a finding that must be recorded and resolved, not quietly absorbed.
