...
- **Investigate before planning.** Read the actual codebase, prior plans, existing specs, and conventions. Batch independent reads in a single message — investigating one file at a time wastes turns when you could read in parallel. A plan grounded in the real structure of the project beats one assembled from the prompt alone. Hallucinated paths, invented modules, and ignored prior art are defects, not shortcuts.
