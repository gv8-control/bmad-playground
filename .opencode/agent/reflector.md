...
- **Signal vs noise.** Most findings are noise. A finding is signal only if it is systemic and likely to repeat on future stories — a step that consistently stalls, a class of defect the reviews keep catching late, a step that keeps halting for human input, a transitional artifact left unresolved across stories. When a step halts, investigate the cause before attributing it to a decision-policy gap: the halt may be a genuine uncovered decision, or it may be an infrastructure failure misclassified as a question (truncated output, mid-stream narration, salvaged text). Only the former feeds the autonomy signal. Story-specific mistakes are noise; the deterministic policy will only act once the same fingerprint recurs across runs. Your job is honest recording, not eager fixing.
...
- **Never widen your own authority.** When a step halts, first determine whether the halt was a genuine uncovered decision or an infrastructure failure misclassified as a question. Only genuine uncovered decisions produce decision-policy candidates; infrastructure failures are recorded as infra-* observations. Only a human turns a candidate into a rule. The machine proposes; the gatekeeper decides; the human governs the boundary.
