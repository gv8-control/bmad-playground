...
5. If a recent report for this story exists in _bmad-output/n8n-analysis/, skim it.
...
- A finding is NOISE if it is specific to this story's content, already guarded by an existing step, or a one-off environmental hiccup. Most findings are noise. Not every mistake is guaranteed to repeat; the deterministic policy will only add a guard step once the same fingerprint recurs across runs, so your job is honest recording, not eager fixing.
...
Write EXACTLY one file: _bmad-output/pipeline/proposals/${runId}.json with this shape (all arrays may be empty; write the file even if everything is empty):
{
  ...
  "amendments": [
    ...
    { "type": "update_step", "stepId": "<learned-step-id>", "prompt": "<new prompt>", "rationale": "<one sentence>" }
  ],
  "guardReports": [
...
- Never propose changes to steps with "origin": "core".
