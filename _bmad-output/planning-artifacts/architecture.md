...
6. **Anthropic/Claude Agent SDK API key:** `ANTHROPIC_API_KEY` is held only in `apps/agent-be`'s Railway environment and never injected into Daytona sandboxes. The agent runs in the host `apps/agent-be` process via the Claude Agent SDK `query()` call (DP-2, Story 3.3), so the key never crosses the sandbox boundary. For sandbox-resident agent execution (Epic 6), `apps/agent-be` exposes a streaming Anthropic proxy endpoint (`@Public()`, `x-api-key` header injection, SSE passthrough) that injects the key server-side — sandboxes call the proxy instead of `api.anthropic.com` directly, so the key is never exposed to sandbox-resident code. The per-user `GITHUB_TOKEN` remains sandbox-injected (host-mediated git operations via the Daytona process API are a post-MVP structural alternative). `networkAllowList` egress restriction on every sandbox (GitHub, the Anthropic proxy, and required package registries only; `networkBlockAll` for everything else) remains mandated as NFR-S1/S2's network-isolation intent (see Technical Constraints).
