...
- On success, the PAT is stored encrypted at rest; it is never returned to the client after initial submission.
...
- Commits produced in a user's session are attributed to that user's platform identity (name and email). The PAT is used for HTTPS transport only; it does not appear in the git commit record.
...
- **Agent-initiated commit failure handling.** If the Agent's git commit fails during a Conversation, no error is surfaced to the user and no retry is attempted. Manual save (FR-15) is the user's instrument for capturing in-progress work; agent-initiated commit failure is a known silent failure mode in MVP.
...
- **NFR-S5 (PAT storage):** Fine-grained GitHub PATs are encrypted when stored on the platform and never returned to the client after initial submission.
...
- **A-6:** Fine-grained GitHub PAT is acceptable onboarding friction for MVP. GitHub App integration is the post-MVP replacement; trigger is PAT friction identified as a material activation blocker in beta. (§6.2)
