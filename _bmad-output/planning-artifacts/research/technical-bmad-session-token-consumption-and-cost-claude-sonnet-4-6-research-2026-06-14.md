...
All BMAD skill file sizes were measured directly on 2026-06-14 using `wc -c` in the local repository at `/workspaces/{project}/.claude/skills/`. Results are exact byte counts; token counts derived using 3.7 chars/token ratio (within 5–15% of actual Claude tokenization).
...
The epics/stories phase dominates artifact _count_ — one `bmad-create-epics-and-stories` session produces 10–20 individual story files. For session _cost_, research and architecture sessions are the heaviest ($1.50–$4.05 each vs. $0.40–$0.65 for brainstorming or short review skills).
