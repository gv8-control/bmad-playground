...
- Rate-limit 403 still conflated with credential failure [`apps/web/src/lib/repository-validation.ts`] — narrower instance of the 403-conflation issue already deferred post-MVP in the story 1.6 review entry above: a GitHub primary/secondary rate limit 403 throws `CredentialFailureError`, marks the credential failed, and tells the user to re-auth. When the post-MVP fix lands, distinguish rate-limit 403s (`x-ratelimit-remaining: 0` header / "rate limit" message) from genuine auth 403s.
