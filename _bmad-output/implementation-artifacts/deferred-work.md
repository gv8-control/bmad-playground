...
## Deferred from: code review of 4-2-provision-the-railway-project-with-postgres-for-apps-agent-be (2026-07-12)

- Token parser in `railway-project-structure.integration.spec.ts:23` doesn't strip surrounding quotes from `.env.local` values — `RAILWAY_TOKEN="abc123"` would include literal quotes in the parsed token, causing a 401. Not an immediate issue: `.env.local` doesn't use quotes currently. Robustness improvement for future environments that might use quoted dotenv values. [`apps/agent-be/test/integration/railway-project-structure.integration.spec.ts:23`]
- `data.serviceInstance` can be null from the Railway API — accessing `.rootDirectory` throws TypeError instead of a clear assertion failure. Test fails correctly, just with a less clear error message. Diagnostic improvement only. [`apps/agent-be/test/integration/railway-project-structure.integration.spec.ts:154`]
- `JSON.parse(rawVars)` throws SyntaxError if Railway returns a non-JSON string for the variables query. Test fails correctly on malformed API response. Diagnostic improvement only. [`apps/agent-be/test/integration/railway-project-structure.integration.spec.ts:180`]
- `response.json()` throws SyntaxError if Railway API returns a non-JSON body (e.g., HTML error page from CDN/proxy). Test fails correctly. Diagnostic improvement only. [`apps/agent-be/test/integration/railway-project-structure.integration.spec.ts:52`]
- `consistency=cached` in `workspaceMount` in `.devcontainer/devcontainer.json` is a macOS-specific Docker Desktop flag. On Windows it can cause file-sync delays. Pre-existing from merged PR #18, not introduced by story 4.2. [`.devcontainer/devcontainer.json`]

## Deferred from: project-context pattern audit of 4-2-provision-the-railway-project-with-postgres-for-apps-agent-be (2026-07-12)

Pattern audited: Secret-aware test assertions — `toHaveProperty` dumps the full object on failure (added to `project-context.md`). Jest's `expect(obj).toHaveProperty('key')` / `.not.toHaveProperty('key')` prints the entire object on assertion failure, leaking secrets into CI logs. Use `expect(Object.keys(obj)).toContain('key')` / `.not.toContain('key')` instead. The following test files use `toHaveProperty` / `not.toHaveProperty` on objects that contain or might contain (on regression) secrets:

- `expect(vars).toHaveProperty('DATABASE_URL')` — `vars` contains `DATABASE_URL` (with embedded password), `PGPASSWORD`, `POSTGRES_PASSWORD` from the Railway `variables` GraphQL query. On failure, all credentials print to CI logs. Already tracked as an NFR finding in the story (NFR Findings, MEDIUM). [`apps/agent-be/test/integration/railway-project-structure.integration.spec.ts:196`]
- `expect(result).not.toHaveProperty('accessToken')` / `.not.toHaveProperty('encryptedToken')` / `.not.toHaveProperty('token')` — `result` is the `GitUserConfig` returned by `getGitIdentity()`. If a regression introduces a token field into the return shape, the `.not.toHaveProperty` assertion fails and prints the full `result` including the token value. [`apps/web/src/actions/git-identity.actions.spec.ts:134-136`]
- `expect(createArg).not.toHaveProperty('env')` — `createArg` is the Daytona `create()` args. If a regression introduces `env: { ANTHROPIC_API_KEY: ... }`, the assertion fails and prints the full `createArg` including the key. Prescribed by the existing regression-guard rule (`project-context.md` line 241) which pre-dates this pattern; migrate to `Object.keys(createArg).not.toContain('env')` when a story touches this test. [`apps/agent-be/src/sandbox/sandbox.service.nfr-s1.spec.ts:61`]
