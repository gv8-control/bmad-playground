...
- Credential health flips to `failed` within one git operation cycle of a 401, with no silent failures; 403 responses are classified (rate limit, org restriction, permission denial) and do not mark the credential as failed. Re-authorization restores `healthy` without disconnecting the repository. Every credential lookup passes a tenant authorization check before resolution — tokens never resolve across users. (Visual failed-status display on the Project Map and real-time SSE propagation are out of scope here — Epic 2 and Epic 3 respectively.)
