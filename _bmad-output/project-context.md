...
- **Canonical depth-1 page header structure (breadcrumb inline + border-b):** depth-1 pages (pages rendering `<Breadcrumb />`) use a canonical header: `<header className="flex-shrink-0 border-b border-surface-raised pt-6 pb-4 px-8"><div className="flex items-center gap-3"><Breadcrumb /><h1 tabIndex={-1} className="text-xl font-semibold text-text-1">Title</h1></div></header>`. The Breadcrumb renders inline beside the h1 (not stacked above), padding lives on the `<header>` (not the h1), and the header has `border-b border-surface-raised`. Depth-0 pages (project-map, no Breadcrumb) do NOT get the border-b divider. `loading.tsx` files must replicate this header structure (including Breadcrumb, border-b, padding, flex wrapper). Established in Story 5.2 across 4 page.tsx + 1 loading.tsx.
- **Visually-hidden h1 (`sr-only`) for headerless pages:** when a mockup omits a visible page header entirely (no Breadcrumb, no visible h1), replace the visible h1 with `<h1 tabIndex={-1} className="sr-only">Title</h1>` — `sr-only` hides it visually while preserving the `tabIndex={-1}` that `AppShell` route-focus management requires. Place it as the first child of the page root so it's available on mount (not behind a Suspense boundary). This is the documented deviation from the canonical depth-1 header structure (above) for mockup-faithful headerless pages. Established in `conversations/new/page.tsx` (Story 5.3).
...
- **Export `react-markdown` `components` object for direct unit testing:** when a component using `react-markdown` with custom `components` overrides needs unit tests on those overrides (e.g. focus ring on `a` elements), but `react-markdown` is mocked at file level (so no rendered `<a>` exists to query), export the `components` object from the module and test each override function directly. See `AgentMessage.tsx` `export const markdownComponents` (Story 5.3 — debug log finding).
...
- **Disabled accent buttons use muted-surface tokens, not `opacity-50`:** buttons with `bg-accent` in their active state use `disabled:bg-text-3 disabled:text-text-2 disabled:border disabled:border-border` for the disabled state (DESIGN.md line 437), NOT `disabled:opacity-50` — `opacity-50` dims the accent color itself, making a disabled button look like a faded active one rather than deliberately disabled. Applies to filled/accent buttons only; secondary text/ghost buttons may still use `disabled:opacity-50`. Established in `ChatInput.tsx` Send button (Story 5.3, AC-4 — DP-2: DESIGN.md wins over mockups).
