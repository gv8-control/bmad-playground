...
describe('POST /api/internal/test/seed-user', () => {
  ...
  it('[P0] upserts with the correct where/update/create payload', async () => {
    mockUserUpsert.mockResolvedValue({ id: 'usr_xyz', githubId: 'gh_2', githubLogin: 'bob' });
    await POST(makeRequest({ githubId: 'gh_2', githubLogin: 'bob' }));
    expect(mockUserUpsert).toHaveBeenCalledWith({
      where: { githubId: 'gh_2' },
      update: { githubLogin: 'bob', name: null },
      create: { githubId: 'gh_2', githubLogin: 'bob', name: null, email: null },
    });
  });

  it('[P1] treats missing name as null', async () => {
    mockUserUpsert.mockResolvedValue({ id: 'usr_1', githubId: 'gh_3', githubLogin: 'carol' });
    await POST(makeRequest({ githubId: 'gh_3', githubLogin: 'carol' }));
    expect(mockUserUpsert).toHaveBeenCalledWith(
...
describe('DELETE /api/internal/test/seed-user', () => {
  ...
  it('[P0] calls deleteMany with the correct where clause', async () => {
    mockUserDeleteMany.mockResolvedValue({ count: 1 });
    await DELETE(makeRequest({ githubId: 'gh_99' }));
    expect(mockUserDeleteMany).toHaveBeenCalledWith({ where: { githubId: 'gh_99' } });
  });
